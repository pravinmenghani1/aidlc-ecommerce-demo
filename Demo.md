...
curl -sL https://github.com/pravinmenghani1/aidlc-workflows/releases/download/v1.0.0/aidlc-workflows.zip \
  -o /tmp/aidlc.zip
unzip -o /tmp/aidlc.zip -d /tmp/aidlc-release

cp -R /tmp/aidlc-release/aidlc-workflows/aidlc-rules/aws-aidlc-rules .kiro/steering/
cp -R /tmp/aidlc-release/aidlc-workflows/aidlc-rules/aws-aidlc-rule-details .kiro/
cp /tmp/aidlc-release/aidlc-workflows/.kiro/steering/aidlc-trigger.md .kiro/steering/

rm -rf /tmp/aidlc.zip /tmp/aidlc-release

# 3. Verify setup
ls .kiro/steering/
# Expected: aws-aidlc-rules/  aidlc-trigger.md
```
...
1. **Pre-provision**: Have participants run the full Pre-Setup block above — it uses a direct download from your release and takes under a minute.
